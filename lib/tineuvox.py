...
class RGBNet(nn.Module):
    def __init__(
        ...
        self.views_linears = nn.Sequential(
            nn.Linear(W + self.input_ch_views, W // 2),
            nn.ReLU(),
            nn.Linear(W // 2, self.output_ch),
        )
...
class TiNeuVox(torch.nn.Module):
    ...
    def scale_volume_grid(self, num_voxels):
        ...
        self._set_grid_resolution(num_voxels)
        print(
            "TiNeuVox: scale_volume_grid scale world_size from",
            ori_world_size,
            "to",
            self.world_size,
        )
...
class Alphas2Weights(torch.autograd.Function):
    ...
    def forward(ctx, alpha, ray_id, N):
        (
            ...
            i_start,
            i_end,
        ) = render_utils_cuda.alpha2weight(alpha, ray_id, N)
...
def get_rays(
    H, W, K, c2w, inverse_y=False, flip_x=False, flip_y=False, mode="center"
):
    i, j = torch.meshgrid(
...
def get_training_rays_flatten(rgb_tr_ori, times, train_poses, HW, Ks, ndc):
    ...
    imsz = []
    top = 0
    for c2w, img, (H, W), K, time_one in zip(
        train_poses, rgb_tr_ori, HW, Ks, times
    ):
...
def get_training_rays_in_maskcache_sampling(
    rgb_tr_ori, times, train_poses, HW, Ks, ndc, model, render_kwargs
):
    ...
    eps_time = time.time() - eps_time
    print(
        "get_training_rays_in_maskcache_sampling: finish (eps time:",
        eps_time,
        "sec)",
    )
    return rgb_tr, times_tr, rays_o_tr, rays_d_tr, viewdirs_tr, imsz
