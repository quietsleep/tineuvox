...
class Load_hyper_data:
    def __init__(self, datadir, ratio=0.5, use_bg_points=False, add_cam=False):
        ...
        self.all_time = [
            meta_json[i]["time_id"] / max_time for i in self.all_img
        ]
    ...
    def load_raw(self, idx):
        ...
        rays_dir = (
            torch.tensor(camera.pixels_to_rays(pixels)).float().view([-1, 3])
        )
        rays_ori = (
            torch.tensor(camera.position[None, :]).float().expand_as(rays_dir)
        )
        rays_color = torch.tensor(np.array(image)).view([-1, 3]) / 255.0
        return {
            "rays_ori": rays_ori,
            "rays_dir": rays_dir,
