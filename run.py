...
def config_parser():
    ...
    parser.add_argument(
        "--fre_test", type=int, default=30000, help="frequency of test"
    )
    parser.add_argument(
        "--step_to_half",
        type=int,
...
def render_viewpoints_hyper(
    ...
    for i in tqdm(idx):
        ...
        render_result_chunks = [
            {
                k: v
                for k, v in model(
                    ro, rd, vd, ts, cams, **render_kwargs
                ).items()
                if k in keys
            }
            for ro, rd, vd, ts, cams in zip(
                rays_o.split(bacth_size, 0),
                rays_d.split(bacth_size, 0),
                viewdirs.split(bacth_size, 0),
                time_one.split(bacth_size, 0),
                cam_one.split(bacth_size, 0),
            )
        ]
        render_result = {
            k: torch.cat([ret[k] for ret in render_result_chunks]).reshape(
                data_class.h, data_class.w, -1
            )
        ...
        if eval_psnr:
            p = -10.0 * np.log10(np.mean(np.square(rgb - rgb_gt)))
            psnrs.append(p)
            rgbs_tensor.append(
                torch.from_numpy(np.clip(rgb, 0, 1)).reshape(
                    -1, data_class.h, data_class.w
                )
            )
            rgbs_gt_tensor.append(
                torch.from_numpy(np.clip(rgb_gt, 0, 1)).reshape(
                    -1, data_class.h, data_class.w
                )
            )
        if i == 0:
    ...
    if eval_psnr:
        ...
        ms_ssims = ms_ssim(
            rgbs_gt_tensor, rgbs_tensor, data_range=1, size_average=True
        )
    if len(psnrs):
...
def render_viewpoints(
    ...
    for i, c2w in enumerate(tqdm(render_poses)):
        ...
        render_result_chunks = [
            {
                k: v
                for k, v in model(ro, rd, vd, ts, **render_kwargs).items()
                if k in keys
            }
...
def compute_bbox_by_cam_frustrm(
    args, cfg, HW, Ks, poses, i_train, near, far, **kwargs
):
...
def compute_bbox_by_cam_frustrm_hyper(args, cfg, data_class):
    ...
    for i in tqdm(data_class.i_train, desc="Computing bbox"):
        ...
        pts_nf = torch.stack(
            [
                rays_o + viewdirs * data_class.near,
                rays_o + viewdirs * data_class.far,
            ]
        )
        xyz_min = torch.minimum(xyz_min, pts_nf.amin((0, 1, 2)))
        xyz_max = torch.maximum(xyz_max, pts_nf.amax((0, 1, 2)))
...
def scene_rep_reconstruction(
    ...
    if cfg.data.dataset_type != "hyper_dataset":
        ...
        times_i_train = times[i_train].to(
    ...
    def gather_training_rays_hyper():
        ...
        for i in tqdm(data_class.i_train, desc="Load training"):
            ...
            if data_class.add_cam:
                cam_tr[top : top + n] = (
                    cam_tr[top : top + n] * data_class.all_cam[i]
                )
            times_tr[top : top + n] = (
                times_tr[top : top + n] * data_class.all_time[i]
            )
            ...
            top += n
        assert top == N
        index_generator = tineuvox.batch_indices_generator(
    ...
    def gather_training_rays():
        ...
        if cfg_train.ray_sampler == "in_maskcache":
            ...
            (
                ...
                viewdirs_tr,
                imsz,
            ) = tineuvox.get_training_rays_in_maskcache_sampling(
            ...
            (
                ...
                viewdirs_tr,
                imsz,
            ) = tineuvox.get_training_rays_flatten(
                rgb_tr_ori=rgb_tr_ori,
                times=times_i_train,
                train_poses=poses[i_train],
                HW=HW[i_train],
    ...
    for global_step in pbar:
        ...
        if global_step in cfg_train.pg_scale:
            n_rest_scales = (
                len(cfg_train.pg_scale)
                - cfg_train.pg_scale.index(global_step)
                - 1
            )
        ...
        # random sample rays
        if (
            cfg_train.ray_sampler in ["flatten", "in_maskcache"]
            or cfg.data.dataset_type == "hyper_dataset"
        ):
            ...
            if cfg.data.dataset_type == "hyper_dataset":
                ...
                if data_class.use_bg_points == True:
                    sel_idx = torch.randint(
                        data_class.bg_points.shape[0], [cfg_train.N_rand // 3]
                    )
        ...
        loss = cfg_train.weight_main * F.mse_loss(
        ...
        if (
            ...
            if cfg_train.weight_tv_feature > 0:
                model.feature_total_variation_add_grad(
                    cfg_train.weight_tv_feature / len(rays_o),
                    global_step < cfg_train.tv_feature_before,
                )
        optimizer.step()
        ...
        if global_step % (args.fre_test) == 0:
            ...
            testsavedir = os.path.join(
                cfg.basedir, cfg.expname, f"{global_step}-test"
            )
    ...
    if global_step != -1:
        ...
        print(
            "scene_rep_reconstruction : saved checkpoints at", last_ckpt_path
        )
...
def train(args, cfg, data_dict=None):
    ...
    eps_time_str = (
        f"{eps_loop//3600:02.0f}:{eps_loop//60%60:02.0f}:{eps_loop%60:02.0f}"
    )
...
if __name__ == "__main__":
    ...
    if args.render_train:
        ...
        imageio.mimwrite(
            os.path.join(testsavedir, "train_video.rgb.mp4"),
            utils.to8b(rgbs),
            fps=30,
            quality=8,
        )
        imageio.mimwrite(
            os.path.join(testsavedir, "train_video.disp.mp4"),
    ...
    if args.render_test:
        ...
        imageio.mimwrite(
            os.path.join(testsavedir, "test_video.rgb.mp4"),
            utils.to8b(rgbs),
            fps=30,
            quality=8,
        )
        imageio.mimwrite(
            os.path.join(testsavedir, "test_video.disp.mp4"),
            utils.to8b(disps / np.max(disps)),
            fps=30,
            quality=8,
        )
    ...
    if args.render_video:
        if cfg.data.dataset_type != "hyper_dataset":
            testsavedir = os.path.join(
                cfg.basedir, cfg.expname, f"render_video_{ckpt_name}_time"
            )
            os.makedirs(testsavedir, exist_ok=True)
            rgbs, disps = render_viewpoints(
